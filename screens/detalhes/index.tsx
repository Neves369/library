...
const Detalhes = ({ route, navigation }: any) => {
    ...
    const { changeFontSize, goToLocation, getCurrentLocation } = useReader();
    ...
    useEffect(() => {
       console.log("Localização: ", activeLocation)
       saveOnStorage()
    }, [activeLocation]);

    async function saveOnStorage() {
        if(book){
            await AsyncStorage.setItem(`@Usuario:${book.nome}`, JSON.stringify(activeLocation))
        }
    }

    async  function loadStorageData(book){
        let storageLocation = await AsyncStorage.getItem(`@Usuario:${book.nome}`)
        console.log("Tem: ", storageLocation)
        if(storageLocation){
            setActiveLocation(JSON.parse(storageLocation));
        }
    }
            ...
            <SafeAreaView>
                <ReaderProvider>
                    <Reader
                      ...
                      width={width}
                      height={height}
                      initialLocation={activeLocation?.end.cfi}
                      onLocationChange={(totalLocations, currentLocation, pageList)=> {setActiveLocation(currentLocation)}}
                      onReady={(totalLocations, currentLocation, pageList)=> {setAllLocations(totalLocations)}}
                      onPress={()=>{ShowMenu(!show)}}
                      renderLoadingComponent={renderLoading}
                      onDisplayError={()=>{console.log("Deu ruim")}}
                    />
                    {
                        show?
                            <>
                                <BookMenuTop>
                                    <View style={{flex: 1, alignItems: 'center', justifyContent: 'flex-end', paddingBottom: 15}}>
                                        <AntDesign name="arrowleft" size={24} color="white" onPress={() =>{setShowBook(false)}}/>
                                    </View>
                                ...
                                <BookMenuBottom>
                                    ...
                                    <View style={{flex: 3, justifyContent: 'center'}}>
                                        <Slider
                                            value={activeLocation.end.percentage}
                                            onValueChange={() =>{}}
                                        />
                                    </View>
                                    <View style={{flex: 1, alignItems: 'center', justifyContent: 'center'}}> 
                                        <Text style={{color: 'white'}}>
                                            {`${activeLocation.start.location}/${allLocations}`}
                                        </Text>
                                    </View>
                                </BookMenuBottom>
                            </>
